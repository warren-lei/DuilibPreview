...
namespace DuiLib {
	...
	LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		if( uMsg == WM_CREATE ) {
			...
			m_pLayout->SetBorderSize(n_intembordersize);
	...
	else if( _tcscmp(pstrName, _T("intembordersize")) == 0 ) n_intembordersize = atoi((const char *)pstrValue);

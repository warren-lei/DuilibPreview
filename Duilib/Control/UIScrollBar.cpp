...
namespace DuiLib
{
	...
	{
		//计算ScrollBar的背景大小
		RECT scrollBarSize(m_rcItem);
		scrollBarSize.top += (m_rcButton1.bottom - m_rcButton1.top);
		scrollBarSize.bottom += (m_rcButton2.bottom - m_rcButton2.top);
		/*改变m_sBkDisabledImage \
				m_sBkPushedImage \
				m_sBkHotImage \
				m_sBkNormalImage \
		的属性*/
		UpdateDestValue(m_sBkDisabledImage, scrollBarSize);
		UpdateDestValue(m_sBkPushedImage, scrollBarSize);
		UpdateDestValue(m_sBkHotImage, scrollBarSize);
		UpdateDestValue(m_sBkNormalImage, scrollBarSize);
	...
	void CScrollBarUI::FetchDestValue(const LPCTSTR destStr, RECT &dest)
	{
#ifdef _UNICODE
		...
		int arrInt[4] = { 0 };  //用于临时存放提取到的值
		for (auto temp : vec)
		{
			if (string::npos != temp.find(_T("dest=")))
			{
				//把dest属性中的数个数字提取出来
				int arrIntIndex(0);
				bool numFlag(false);
				for (auto c : temp)
				{
					if ('0' <= c && c <= '9')
					{
						if (numFlag)
						{
							arrInt[arrIntIndex] = arrInt[arrIntIndex] * 10 + c - '0';
						}
						else
							arrInt[arrIntIndex] = c - '0';
						numFlag = true;
					}
					else
					{
		...
		dest.top = arrInt[1];	
		dest.right = arrInt[2];
		dest.bottom = arrInt[3];
		
	}

	void CScrollBarUI::UpdateDestValue(CDuiString &destStr, RECT &destRect)
	{
#ifdef _UNICODE
		typedef std::wstring string;
		typedef std::wistringstream istringstream;
		...
		using std::vector;
		RECT dest;
		FetchDestValue((LPCTSTR)destStr, dest);

		string str((LPCTSTR)destStr);
		istringstream istr(str);
		...
		while (!istr.eof())
		{
			string str;
			istr >> str;
			if (string::npos != str.find(_T("dest=")))
				continue;
			vec.push_back(str);
		}
		//把dest的值格式化
		ostringstream outDestStr;
		outDestStr << " dest=\'" << dest.left << ',' << dest.top << ',' << dest.right << ',' << destRect.bottom - destRect.top << "\' ";
		string tempDestStr = outDestStr.str();
		destStr.Empty();
		TCHAR sp = ' ';
		//把vec中的值存放到BkImage中
		for (auto temp : vec)
		{
			destStr += const_cast<LPCTSTR>(temp.c_str());
			destStr += sp;
		}
		destStr += const_cast<LPCTSTR>(tempDestStr.c_str());
		destStr += sp;
	}
}